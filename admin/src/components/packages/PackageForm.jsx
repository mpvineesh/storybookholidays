...
const PackageForm = React.forwardRef(
  ({ initialPackage, isSaving, errorMessage, onSubmit, onCancel }, formRef) => {
    ...
    React.useEffect(() => {
      if (initialPackage) {
        setForm({
          ...
          region: initialPackage.region || 'Kerala',
          duration: initialPackage.duration || '',
          shortDescription: initialPackage.shortDescription || '',
          contentHtml: initialPackage.contentHtml || '',
        });
        setImageFile(null);
        setImagePreviewUrl(initialPackage.imageUrl || '');
      } else {
        setForm(emptyForm());
        setImageFile(null);
        setImagePreviewUrl('');
    ...
    return (
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-5" id="package-form">
            ...
            <label className="inline-flex items-center gap-2 cursor-pointer rounded-lg border border-border bg-white px-3 h-9 text-sm text-ink hover:bg-slate-50">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setImageFile(file);
                  if (!file && initialPackage?.imageUrl) {
                    setImagePreviewUrl(initialPackage.imageUrl);
                  } else if (!file) {
                    setImagePreviewUrl('');
                  }
        ...
        <div>
          ...
          <RichTextEditor
            ...
            placeholder="Day by day itinerary, inclusions, etc."
          />
        </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          <Button type="submit" disabled={isSaving}>
            ...
            {isSaving ? 'Saving…' : initialPackage ? 'Update package' : 'Create package'}
          </Button>
        </div>
      </form>
    );
