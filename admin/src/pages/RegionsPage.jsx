...
import RegionForm from '@/components/regions/RegionForm.jsx';
import {
  createRegion,
  deleteRegion,
  listAdminRegions,
  updateRegion,
} from '@/lib/api/regionsApi';
...
const RegionsPage = () => {
  ...
  const load = React.useCallback(async () => {
    ...
    setErrorMessage('');
    try {
      const response = await listAdminRegions();
  ...
  const filtered = React.useMemo(() => {
    ...
    return items.filter((entry) =>
      [entry.title, entry.region, entry.slug, entry.description]
        .filter(Boolean)
  ...
  const handleSubmit = async (form, imageFile) => {
    ...
    try {
      if (editing?._id) {
        await updateRegion(editing._id, form, imageFile);
        setSuccessMessage('Region updated.');
      } else {
        await createRegion(form, imageFile);
        setSuccessMessage('Region created.');
      }
      await load();
      setDrawerOpen(false);
      setEditing(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteRegion(confirmTarget._id);
      setSuccessMessage('Region deleted.');
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-ink">Regions</h2>
          <p className="text-sm text-ink-muted mt-1">
            Manage region cards, images, URLs, and availability.
          </p>
        </div>
        ...
        <CardBody className="p-0">
          <div className="border-b border-border px-4 py-3 flex items-center gap-3 flex-wrap">
            <div className="relative flex-1 max-w-md">
              ...
              />
              <Input
                placeholder="Search by title, region, slug..."
                value={search}
                  ...
                  <tr>
                    <td colSpan={4} className="px-4 py-10 text-center text-ink-muted">
                      {search.trim()
                        ...
                        : 'No regions yet. Click "Add region" to create the first one.'}
                    </td>
                  </tr>
                ) : (
                  filtered.map((entry) => (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-ink-subtle line-clamp-1">
                              {entry.description || entry.tagline || 'No description'}
                            </p>
                            <p className="mt-1 text-xs font-mono text-ink-subtle">
                              {entry.region}
                            </p>
                          </div>
                        </div>
      ...
      <Drawer
        ...
        description="Configure the card shown on the public region chooser."
      >
        <RegionForm
          initialRegion={editing}
          isSaving={isSaving}
      ...
      <ConfirmDialog
        ...
        description={
          confirmTarget
            ? `This removes "${confirmTarget.title}" from the admin region list. Existing content records are not deleted.`
            : ''
        }
        confirmLabel="Delete"
